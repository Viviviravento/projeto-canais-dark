...
from ai.fabrica.core.contracts import list_contracts, load_schema, validate_contract
from ai.fabrica.core.costs import CostLedger, PricePoint
from ai.fabrica.core.gates import GatePolicy
from ai.fabrica.core.genesis import ChannelThesis, exploratory_theses, thesis_ready_for_pilot
from ai.fabrica.core.incidents import IntegrationIncident
from ai.fabrica.core.media import select_cost_optimized_eligible
from ai.fabrica.core.metrics import ImmutableRecordStore
from ai.fabrica.core.pipeline import CHANNEL_STAGES, VIDEO_STAGES, FactoryRun, validate_run_payload
from ai.fabrica.core.qa import audit_package
...
class ContractTests(unittest.TestCase):
    def test_all_public_schemas_are_valid(self) -> None:
        self.assertEqual(len(list_contracts()), 11)
...
class PipelineTests(unittest.TestCase):
    def test_channel_and_video_have_all_fixed_stages(self) -> None:
        self.assertEqual(tuple(FactoryRun("c1", "channel").stages), CHANNEL_STAGES)
        self.assertEqual(tuple(FactoryRun("v1", "video").stages), VIDEO_STAGES)
    ...
    def test_mukbang_dry_run_crosses_same_macro_stages(self) -> None:
        path = Path(__file__).resolve().parents[3] / "00_incubadora" / "mukbang-ensaio-seco" / "run.json"
    ...
    def test_qa_checks_scenes_not_frames(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            report = audit_package({
                ...
                "end_screen": {"required": True, "duration_seconds": 10, "minimum_seconds": 5, "maximum_seconds": 20, "slots": 1},
            })
