...
foreach ($paragrafo in $paragrafos) {
    ...
    if ($candidato.Length -gt 1350 -and $atual) { $partes.Add($atual); $atual = $paragrafo } else { $atual = $candidato }
}
...
$rotulos = ''
for ($i = 0; $i -lt $partesGeradas.Count; $i++) {
    $entradas.Add('-i'); $entradas.Add($partesGeradas[$i].FullName)
    $rotulos += "[${i}:a]"
}
$filtro = "${rotulos}concat=n=$($partesGeradas.Count):v=0:a=1[a]"
& ffmpeg -hide_banner -y @entradas -filter_complex $filtro -map '[a]' -c:a libmp3lame -b:a 192k $master
