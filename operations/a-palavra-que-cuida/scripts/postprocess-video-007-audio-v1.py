...
AUDIO_DIR = ROOT / "operations" / "a-palavra-que-cuida" / "05_audio" / "video-007" / "v1"
FFMPEG = ROOT / "tools" / "ffmpeg" / "bin" / "ffmpeg.exe"
FFPROBE = ROOT / "tools" / "ffmpeg" / "bin" / "ffprobe.exe"
PREFIX = "video-007-narracao-v1"
TEMPO = 1.07
...
def main() -> None:
    ...
    tempo_wav, master_wav, review_mp3 = (AUDIO_DIR / f"{PREFIX}-1.07x-raw.wav", AUDIO_DIR / f"{PREFIX}-1.07x-master.wav", AUDIO_DIR / f"{PREFIX}-1.07x-review.mp3")
    alignment_path, audit_path = AUDIO_DIR / "master-alignment-1.07x.json", AUDIO_DIR / "postprocess-1.07x.json"
    ...
    for result in report["results"]:
        ...
        starts, ends = alignment.get("character_start_times_seconds") or [], alignment.get("character_end_times_seconds") or []
        blocks.append({"block_id": result["block_id"], "start_seconds": round(offset, 6), "end_seconds": round(offset + result["duration_seconds"] / TEMPO, 6), "characters": alignment.get("characters") or [], "character_start_times_seconds": [round(offset + value / TEMPO, 6) for value in starts], "character_end_times_seconds": [round(offset + value / TEMPO, 6) for value in ends]})
        offset += result["duration_seconds"] / TEMPO
    master_duration = duration(master_wav)
    alignment_path.write_text(json.dumps({"version": "video-007-master-alignment-1.07x-v1", "tempo": TEMPO, "pitch": "preserved", "source_generation_report": "resultado-geracao.json", "blocks": blocks, "expected_duration_seconds": round(offset, 6), "master_duration_seconds": round(master_duration, 6)}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    audit_path.write_text(json.dumps({"version": "video-007-audio-postprocess-1.07x-v1", "source": source.name, "tempo": TEMPO, "pitch": "preserved", "raw_duration_seconds": round(duration(source), 6), "master_duration_seconds": round(master_duration, 6), "files": {"tempo_wav": tempo_wav.name, "master_wav": master_wav.name, "review_mp3": review_mp3.name, "alignment": alignment_path.name}, "loudness": {"pre_normalization": measured, "master": normalized, "target_integrated_lufs": -16.0, "target_true_peak_dbfs": -1.5}}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
