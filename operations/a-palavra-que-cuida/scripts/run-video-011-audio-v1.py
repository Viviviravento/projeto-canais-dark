...
MANIFEST = CHANNEL_ROOT / "02_roteiros" / "video-011-generation-manifest-v1.json"
QUOTE_PATH = CHANNEL_ROOT / "06_edicao" / "video-011" / "custos" / "elevenlabs-v1.json"
EXECUTION_PATH = CHANNEL_ROOT / "06_edicao" / "video-011" / "custos" / "elevenlabs-v1-execution.json"
PREFLIGHT_PATH = CHANNEL_ROOT / "06_edicao" / "video-011" / "preflight-prosodia-v1.json"
OUTPUT_DIR = CHANNEL_ROOT / "05_audio" / "video-011" / "v1"
...
def command(generate: bool) -> list[str]:
    command_line = [
        ...
        str(OUTPUT_DIR),
        "--output-prefix",
        "video-011-narracao-v1",
        "--review-title",
        "Vídeo 011 — Narração v1",
...
def main() -> None:
    ...
    parser.add_argument("--user-statement", default="áudio aprovado")
    parser.add_argument("--resume-approved-environment-failure", action="store_true")
    ...
    environment_recovery = (
        args.resume_approved_environment_failure
        and quote.get("status") == "failed"
        and execution is not None
        and execution.get("state") == "failed_needs_incident"
        and not (OUTPUT_DIR / "resultado-geracao.json").exists()
    )
    if quote.get("status") != "reserved_pending_user_approval" and not environment_recovery:
    ...
    if not environment_recovery:
        ...
        execution.update({
            "state": "reserved_environment_recovery",
            "recovery_at": now(),
            "recovery_reason": "Missing requests dependency prevented execution before any provider request.",
        })
