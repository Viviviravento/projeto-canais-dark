...
AUDIO_DIR = PROJECT_ROOT / "operations" / "a-palavra-que-cuida" / "05_audio" / "video-006" / "v1"
FFMPEG = PROJECT_ROOT / "tools" / "ffmpeg" / "bin" / "ffmpeg.exe"
FFPROBE = PROJECT_ROOT / "tools" / "ffmpeg" / "bin" / "ffprobe.exe"
TEMPO = 1.05
PREFIX = "video-006-narracao-v1"
...
def loudness(path: Path) -> dict[str, float]:
    result = subprocess.run(
        [str(FFMPEG), "-hide_banner", "-nostats", "-i", str(path), "-af", "loudnorm=I=-16:TP=-1.5:LRA=11:print_format=json", "-f", "null", "NUL"],
        check=True,
        capture_output=True,
        text=True,
    )
...
def main() -> None:
    ...
    tempo_wav = AUDIO_DIR / f"{PREFIX}-1.05x-raw.wav"
    master_wav = AUDIO_DIR / f"{PREFIX}-1.05x-master.wav"
    review_mp3 = AUDIO_DIR / f"{PREFIX}-1.05x-review.mp3"
    alignment_path = AUDIO_DIR / "master-alignment-1.05x.json"
    audit_path = AUDIO_DIR / "postprocess-1.05x.json"

    subprocess.run(
        [str(FFMPEG), "-y", "-i", str(source), "-af", f"atempo={TEMPO}", "-ac", "1", "-ar", "48000", "-c:a", "pcm_s16le", str(tempo_wav)],
        check=True,
        capture_output=True,
    )
    ...
    subprocess.run(
        [str(FFMPEG), "-y", "-i", str(tempo_wav), "-af", normalization, "-ac", "1", "-ar", "48000", "-c:a", "pcm_s16le", str(master_wav)],
        check=True,
        capture_output=True,
    )
    subprocess.run(
        [str(FFMPEG), "-y", "-i", str(master_wav), "-vn", "-c:a", "libmp3lame", "-b:a", "192k", str(review_mp3)],
        check=True,
        capture_output=True,
    )
    ...
    for result in report["results"]:
        alignment_payload = json.loads((AUDIO_DIR / f"{result['block_id']}.alignment.json").read_text(encoding="utf-8"))
    ...
    master_duration = duration(review_mp3)
    alignment_path.write_text(json.dumps({
        "version": "video-006-master-alignment-1.05x-v1",
        "tempo": TEMPO,
        "pitch": "preserved",
    ...
    audit_path.write_text(json.dumps({
        "version": "video-006-audio-postprocess-1.05x-v1",
        "source": source.name,
        "tempo": TEMPO,
        "pitch": "preserved",
