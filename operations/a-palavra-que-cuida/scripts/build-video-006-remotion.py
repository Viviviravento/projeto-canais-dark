...
SOURCE_PROJECT = ROOT / "tools" / "OpenMontage" / "projects" / "a-palavra-que-cuida-video-005"
PROJECT = ROOT / "tools" / "OpenMontage" / "projects" / "a-palavra-que-cuida-video-006"
PUBLIC = ROOT / "tools" / "OpenMontage" / "remotion-composer" / "public" / "video-006"
ALIGNMENT = CHANNEL / "05_audio" / "video-006" / "v1" / "master-alignment-1.05x.json"
SCENE_PLAN = CHANNEL / "06_edicao" / "video-006" / "scene-plan-v1.json"
MUSIC = ROOT / "tools" / "OpenMontage" / "projects" / "a-palavra-que-cuida-video-003" / "public" / "music" / "classical-6-jonny-s-mixkit.mp3"

ASSET_ROOTS = {
    "video-006": CHANNEL / "04_assets" / "video-006",
...
def copy_assets(plan: dict) -> None:
    copy_one(CHANNEL / "05_audio" / "video-006" / "v1" / "video-006-narracao-v1-1.05x-master.wav", PUBLIC / "audio" / "narracao-producao.wav")
...
def asset_scene(asset: str) -> dict:
    family, relative = asset.split("/", 1)
    return {"asset": f"video-006/assets/{family}/{relative}", "media": "image", "focus": "center", "shade": 0.045}
...
def build_scenes(plan: dict) -> list[dict]:
    ...
    for item in plan["scripture_cards"]:
        ...
        scenes.append({"id": item["id"], "start": round(float(item["start_seconds"]), 3), "end": round(float(item["end_seconds"]), 3), "asset": f"video-006/assets/scripture/{source.name}", "media": "image", "focus": "center", "shade": 0})
    return scenes
...
def build_project_files() -> None:
    ...
    text = text.replace("Video005", "Video006").replace("video-005", "video-006")
    text = text.replace(
        "Continue por um dos vídeos sugeridos na tela e conte nos comentários o que Gênesis mais falou com você hoje.",
        "Continue por um dos vídeos sugeridos na tela e deixe sua resposta nos comentários.",
    )
    composition.write_text(text, encoding="utf-8")
    root = PROJECT / "RootV2.tsx"
    root.write_text(root.read_text(encoding="utf-8").replace("Video005", "Video006").replace("Video005V1", "Video006V1"), encoding="utf-8")
