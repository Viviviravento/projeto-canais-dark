...
AUDIO_DIR = PROJECT_ROOT / "operations" / "a-palavra-que-cuida" / "05_audio" / "video-005" / "v1"
FFMPEG = PROJECT_ROOT / "tools" / "ffmpeg" / "bin" / "ffmpeg.exe"
FFPROBE = PROJECT_ROOT / "tools" / "ffmpeg" / "bin" / "ffprobe.exe"
TEMPO = 1.05
PREFIX = "video-005-narracao-v1"
...
def main() -> None:
    ...
    for result in report["results"]:
        alignment_payload = json.loads(
            (AUDIO_DIR / f"{result['block_id']}.alignment.json").read_text(encoding="utf-8")
        )
        ...
        blocks.append(
            {
                ...
                "character_start_times_seconds": [
                    round(offset + value / TEMPO, 6) for value in starts
                ],
                "character_end_times_seconds": [
                    round(offset + value / TEMPO, 6) for value in ends
                ],
            }
        )
    ...
    alignment_path.write_text(
        json.dumps(
            {
                "version": "video-005-master-alignment-1.05x-v1",
                "tempo": TEMPO,
                "pitch": "preserved",
    ...
    audit_path.write_text(
        json.dumps(
            {
                "version": "video-005-audio-postprocess-1.05x-v1",
                "source": source.name,
                "tempo": TEMPO,
                "pitch": "preserved",
