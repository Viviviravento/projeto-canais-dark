...
MANIFEST_PATH = PROJECT_ROOT / "operations" / "a-palavra-que-cuida" / "02_roteiros" / "video-004-citacoes-acf-v1.json"
OUTPUT_DIR = PROJECT_ROOT / "operations" / "a-palavra-que-cuida" / "04_assets" / "video-004" / "scripture_cards"
WIDTH = 1920
HEIGHT = 1080
BACKGROUND = "#F5F0E7"
INK = "#26382F"
...
def main() -> None:
    ...
    for item in manifest["direct_acf_citations"]:
        ...
        reference_font = font(52, bold=True)
        body_font, lines = quote_font(draw, item["text"], 1370, 500)
        draw.text((230, 205), "ALMEIDA CORRIGIDA FIEL", fill=ACCENT, font=label_font)
        draw.text((230, 258), item["reference"], fill=INK, font=reference_font)
    ...
    (OUTPUT_DIR / "manifest.json").write_text(json.dumps({"source": str(MANIFEST_PATH.relative_to(PROJECT_ROOT)).replace("\\", "/"), "cards": results}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
