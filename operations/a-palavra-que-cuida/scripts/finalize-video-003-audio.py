...
AUDIO_DIR = OPERATION_ROOT / "05_audio" / "video-003" / "v1"
...
def main() -> None:
    ...
    production = AUDIO_DIR / "video-003-narracao-v1-production.wav"
    review = AUDIO_DIR / "video-003-narracao-v1-production-review.mp3"

    subprocess.run(
        [str(FFMPEG), "-y", "-i", str(source), "-af", f"apad=pad_dur={TAIL_SECONDS}", "-ac", "1", "-ar", "48000", "-c:a", "pcm_s16le", str(production)],
        check=True,
        capture_output=True,
    )
    subprocess.run(
        [str(FFMPEG), "-y", "-i", str(production), "-vn", "-c:a", "libmp3lame", "-b:a", "192k", str(review)],
        check=True,
        capture_output=True,
    )
    ...
    for result in report["results"]:
        ...
        starts = [round(offset + float(value), 6) for value in alignment["character_start_times_seconds"]]
        ends = [round(offset + float(value), 6) for value in alignment["character_end_times_seconds"]]
        ...
        blocks.append({
            ...
            "file_end_seconds": round(offset + float(result["duration_seconds"]), 3),
        })
    ...
    (AUDIO_DIR / "merged-alignment-v1.json").write_text(
    ...
    report["production"] = {
        ...
        "tail_silence_seconds": TAIL_SECONDS,
        "merged_alignment": "merged-alignment-v1.json",
    }
    ...
    qa = {
        ...
        "checks": {
            ...
            "request_stitching_after_first_block": all(item["context_mode"] == "request_stitching" for item in report["results"][1:]),
            "master_loudness_near_minus_16_lufs": -16.5 <= float(report["assembled"]["loudness"]["master_integrated_lufs"]) <= -15.5,
            "true_peak_at_or_below_minus_1_dbfs": float(report["assembled"]["loudness"]["master_true_peak_dbfs"]) <= -1.0,
    ...
    (AUDIO_DIR / "audio-qa-v1.json").write_text(json.dumps(qa, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    review_page = AUDIO_DIR / "avaliacao.html"
    page = review_page.read_text(encoding="utf-8")
    page = page.replace("video-003-narracao-v1-review.mp3", review.name)
    review_page.write_text(page, encoding="utf-8")
    print(json.dumps({"production": str(production), "review": str(review), "duration_seconds": round(production_duration, 3), "qa": qa["automated_state"]}, ensure_ascii=False))
