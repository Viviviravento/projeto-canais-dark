...
SOURCE_AUDIO = ROOT / "operations/a-palavra-que-cuida/05_audio/video-006/v1/video-006-narracao-v1-1.05x-master.wav"
ALIGNMENT = ROOT / "operations/a-palavra-que-cuida/05_audio/video-006/v1/master-alignment-1.05x.json"
WORK_ROOT = ROOT / "operations/a-palavra-que-cuida/06_edicao/video-006/shorts"
PROPS_ROOT = WORK_ROOT / "props"
PUBLIC_AUDIO = ROOT / "tools/OpenMontage/remotion-composer/public/video-006-curtos/audio"
GAP_SECONDS = 0.18
...
SPECS = (
    ShortSpec(
        short_id="01",
        title="Jesus chama você pelo nome na ansiedade",
        hook="Jesus chama você pelo nome",
        segments=(SegmentSpec(
            "A resposta de Jesus não começa com desprezo",
            "maior que a presença dele",
        ),),
        visuals=(
            VisualSpec(9.0, "video-006/assets/video-006/images/h02-marta-servindo-v1.png", "54% 50%"),
            VisualSpec(18.0, "video-006/assets/video-006/images/b02-responsabilidades-a-noite-v1.png", "58% 50%"),
            VisualSpec(27.0, "video-006/assets/video-006/images/h01-maria-em-escuta-v1.png", "30% 50%"),
            VisualSpec(36.0, "video-006/assets/library/cotidiano_brasileiro/p001-peso-mental-v1.png", "62% 50%"),
            VisualSpec(45.0, "video-006/assets/video-006/images/b01-pausa-na-cozinha-com-croche-v1.png", "62% 50%"),
            VisualSpec(54.0, "video-006/assets/library/cotidiano_brasileiro/p001-reflexo-v1.png", "69% 50%"),
            VisualSpec(63.0, "video-006/assets/library/apoio_relacoes/p001-escuta-v1.png", "28% 50%"),
            VisualSpec(72.0, "video-006/assets/library/objetos_cenarios/p001-maos-em-pausa-v1.png", "50% 50%"),
            VisualSpec(999.0, "video-006/assets/video-006/images/h02-marta-servindo-v1.png", "54% 50%"),
        ),
        scriptures=(ScriptureSpec(
            "E respondendo Jesus disse-lhe Marta Marta",
    ...
    ShortSpec(
        short_id="02",
        title="Sobrecarga também precisa de ajuda real",
        hook="Sobrecarga precisa de ajuda real",
        segments=(SegmentSpec(
            "É importante trazer essa passagem para a vida de hoje com responsabilidade",
            "cuidado prático e escuta de Jesus não sejam inimigos",
        ),),
        visuals=(
            VisualSpec(9.0, "video-006/assets/video-006/images/b02-responsabilidades-a-noite-v1.png", "58% 50%"),
            VisualSpec(18.0, "video-006/assets/library/apoio_relacoes/p001-pedir-ajuda-v1.png", "56% 50%"),
            VisualSpec(27.0, "video-006/assets/video-006/images/b03-responsabilidade-compartilhada-v1.png", "30% 50%"),
            VisualSpec(36.0, "video-006/assets/video-006/images/b03-responsabilidade-compartilhada-v1.png", "72% 50%"),
            VisualSpec(45.0, "video-006/assets/library/apoio_relacoes/p001-escuta-v1.png", "28% 50%"),
            VisualSpec(54.0, "video-006/assets/library/apoio_relacoes/p001-escuta-v1.png", "72% 50%"),
            VisualSpec(63.0, "video-006/assets/library/cotidiano_brasileiro/p001-peso-mental-v1.png", "62% 50%"),
            VisualSpec(72.0, "video-006/assets/video-006/images/b01-pausa-na-cozinha-com-croche-v1.png", "62% 50%"),
            VisualSpec(999.0, "video-006/assets/library/objetos_cenarios/p001-maos-em-pausa-v1.png", "50% 50%"),
        ),
        scriptures=(),
    ),
)
...
def make_visuals(spec: ShortSpec, duration: float) -> list[dict[str, Any]]:
    ...
    for visual in spec.visuals:
        ...
        if end <= start:
            break
        result.append({
            "start": round(start, 3), "end": round(end, 3), "asset": visual.asset,
            "focus": visual.focus, "scale": visual.scale, "fit": visual.fit,
        })
        start = end
        if start >= duration:
            break
...
def render_audio(segments: list[tuple[float, float]], output: Path) -> None:
    ...
    for index, (start, end) in enumerate(segments):
        filters.append(f"[0:a]atrim=start={start:.6f}:end={end:.6f},asetpts=PTS-STARTPTS,aresample=48000[a{index}]")
...
def main() -> None:
    ...
    for spec in SPECS:
        ...
        audio_name = f"video-006-curto-{spec.short_id}-narracao.wav"
        ...
        for scripture in spec.scriptures:
            ...
            scriptures.append({
                ...
                "text": scripture.text, "reference": scripture.reference,
            })
        props = {
            "derivativeId": f"video-006-curto-{spec.short_id}", "title": spec.title,
            "platform": "universal", "audioFile": f"video-006-curtos/audio/{audio_name}",
            "spokenDurationSeconds": spoken_duration, "endCardSeconds": 4.0,
            "hook": spec.hook, "hookEndSeconds": 4.2,
            "visuals": make_visuals(spec, spoken_duration), "cues": cues, "scriptures": scriptures,
        }
        props_path = PROPS_ROOT / f"video-006-curto-{spec.short_id}-shorts-e-tiktok.json"
        props_path.write_text(json.dumps(props, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        outputs.append({
            "derivative_id": props["derivativeId"], "title": spec.title,
            "source_windows": source_windows, "spoken_duration_seconds": spoken_duration,
            ...
            "platforms": ["youtube_shorts", "tiktok"], "paid_api_cost_usd": 0,
        })
    manifest = {
        "schema_version": 1, "source_video": "video-006",
        "rendered_editorial_derivatives": outputs, "editorial_derivative_count": 2,
        "physical_mp4_target_count": 2, "paid_api_cost_usd": 0,
        "method": "Narração e imagens canônicas, alinhamento aprovado, recomposição vertical manual em cover e render local.",
    }
