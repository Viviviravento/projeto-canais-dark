...
MANIFEST = CHANNEL_ROOT / "02_roteiros" / "video-008-performance-v1.json"
QUOTE_PATH = CHANNEL_ROOT / "06_edicao" / "video-008" / "custos" / "elevenlabs-v1.json"
EXECUTION_PATH = CHANNEL_ROOT / "06_edicao" / "video-008" / "custos" / "elevenlabs-v1-execution.json"
PREFLIGHT_PATH = CHANNEL_ROOT / "06_edicao" / "video-008" / "preflight-prosodia-v1.json"
OUTPUT_DIR = CHANNEL_ROOT / "05_audio" / "video-008" / "v1"
...
def command(generate: bool) -> list[str]:
    command_line = [sys.executable, str(BUILDER), "--manifest", str(MANIFEST), "--output-dir", str(OUTPUT_DIR), "--output-prefix", "video-008-narracao-v1", "--review-title", "Vídeo 008 — Narração v1", "--disable-request-stitching"]
    if generate:
