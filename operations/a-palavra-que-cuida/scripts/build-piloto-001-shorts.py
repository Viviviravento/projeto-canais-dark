...
PROJECT = PROJECT_ROOT / "tools" / "OpenMontage" / "projects" / "a-palavra-que-cuida-piloto-001"
AUDIO_ROOT = OPERATION_ROOT / "05_audio" / "piloto-001" / "v1"
SOURCE_AUDIO = AUDIO_ROOT / "piloto-001-narracao-v1-master.wav"
WORK_ROOT = OPERATION_ROOT / "06_edicao" / "piloto-001" / "shorts"
...
SPECS = (
    ...
    ShortSpec(
        ...
        visuals=(
            ("images/img-s06-biblia-mesa-v1.png", "52% 54%", 1.14),
            ("images/img-s24-entrega-v1.png", "32% 48%", 1.13),
            ("images/img-s26-oracao-e-acao-v1.png", "50% 50%", 1.14),
            ("images/img-s17-pausa-v1.png", "50% 48%", 1.13),
...
def main() -> None:
    ...
    for spec in SPECS:
        ...
        for platform in ("tiktok", "youtube_shorts"):
            ...
            platform_files[platform] = str(props_path.relative_to(PROJECT_ROOT)).replace("\\", "/")
        ...
        outputs.append(
            {
                ...
                "audio": str(audio_output.relative_to(PROJECT_ROOT)).replace("\\", "/"),
