...
AUDIO_DIR = ROOT / "operations" / "a-palavra-que-cuida" / "05_audio" / "video-010" / "v1"
FFMPEG = ROOT / "tools" / "ffmpeg" / "bin" / "ffmpeg.exe"
FFPROBE = ROOT / "tools" / "ffmpeg" / "bin" / "ffprobe.exe"
PREFIX = "video-010-narracao-v1"
TEMPO = 1.07
...
def main() -> None:
    ...
    alignment_path.write_text(json.dumps({"version": "video-010-master-alignment-1.07x-v1", "tempo": TEMPO, "pitch": "preserved", "source_generation_report": "resultado-geracao.json", "blocks": blocks, "expected_duration_seconds": round(offset, 6), "master_duration_seconds": round(master_duration, 6)}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    audit_path.write_text(json.dumps({"version": "video-010-audio-postprocess-1.07x-v1", "source": source.name, "tempo": TEMPO, "pitch": "preserved", "raw_duration_seconds": round(duration(source), 6), "master_duration_seconds": round(master_duration, 6), "files": {"tempo_wav": tempo_wav.name, "master_wav": master_wav.name, "review_mp3": review_mp3.name, "alignment": alignment_path.name}, "loudness": {"pre_normalization": measured, "master": normalized, "target_integrated_lufs": -16.0, "target_true_peak_dbfs": -1.5}}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
