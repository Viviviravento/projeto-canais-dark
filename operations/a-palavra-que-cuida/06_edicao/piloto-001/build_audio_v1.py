...
MANIFEST_PATH = (
    ROOT / "operations" / "a-palavra-que-cuida" / "02_roteiros" / "piloto-001-performance-v1.json"
)
ENV_PATH = ROOT / "tools" / "OpenMontage" / ".env"
OUTPUT_DIR = ROOT / "operations" / "a-palavra-que-cuida" / "05_audio" / "piloto-001" / "v1"
...
def assemble(blocks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "duration_seconds": round(duration(master_wav), 3),
        "loudness": {
...
def build_review_page(manifest: dict[str, Any], results: list[dict[str, Any]]) -> None:
    context_description = (
        "Continuidade por Request Stitching."
        if any(item.get("context_mode") == "request_stitching" for item in results)
        else "Blocos independentes com contexto textual apenas do trecho seguinte."
    )
    rows = []
...
<body><main>
  ...
  <p>Voz A aprovada, grandes blocos semanticos e {html.escape(context_description)}</p>
...
def main() -> int:
    ...
    parser.add_argument("--resume-missing-blocks", action="store_true")
    parser.add_argument("--regenerate-block")
    parser.add_argument("--disable-request-stitching", action="store_true")
    ...
    if any(path.exists() for path in existing) and not all(path.exists() for path in existing):
        if not args.force and not args.resume_missing_blocks:
            raise RuntimeError(
                "Partial generation found. Use --resume-missing-blocks to continue only missing blocks, or --force to regenerate all blocks."
            )
    ...
    if args.regenerate_block:
        ...
        for index, block in enumerate(blocks):
            audio_path = OUTPUT_DIR / f"{block['id']}.mp3"
            alignment_path = OUTPUT_DIR / f"{block['id']}.alignment.json"
            if args.resume_missing_blocks and audio_path.exists() and alignment_path.exists():
                saved = json.loads(alignment_path.read_text(encoding="utf-8"))
                saved_generation = saved.get("generation")
                if not isinstance(saved_generation, dict) or saved_generation.get("block_id") != block["id"]:
                    raise RuntimeError(f"Cannot safely resume {block['id']}: invalid saved alignment")
                results.append(saved_generation)
                continue
            ...
            result = generate_block(
                ...
                block,
                next_block,
                [] if args.disable_request_stitching else request_ids,
            )
            if not args.disable_request_stitching:
                request_ids.append(result["request_id"])
            results.append(result)

    assembled = assemble(results)
    generation_performed = bool(
        args.force or args.regenerate_block or args.resume_missing_blocks or not reused_generation
    )
    ...
    report = {
        ...
        "request_stitching": not args.disable_request_stitching,
        "revisions": revisions,
        "results": results,
        "assembled": assembled,
    }
