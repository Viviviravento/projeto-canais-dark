...
$Canal = Get-Content -LiteralPath "$Root\operations\a-palavra-que-cuida\08_publicacao\configuracao-canal-v1.json" -Raw -Encoding UTF8 | ConvertFrom-Json
$Video001 = Get-Content -LiteralPath "$Root\operations\a-palavra-que-cuida\08_publicacao\piloto-001-publicacao-v1.json" -Raw -Encoding UTF8 | ConvertFrom-Json
$Video002 = Get-Content -LiteralPath "$Root\operations\a-palavra-que-cuida\08_publicacao\video-002-publicacao-v1.json" -Raw -Encoding UTF8 | ConvertFrom-Json
...
yutu channelBanner insert --channelId $ChannelId --file "$Root\operations\a-palavra-que-cuida\04_assets\branding\banner-youtube-v1.png" --output json --yes
```

O handle e a foto `operations/a-palavra-que-cuida/Imagem Perfil Avatar.png` continuam no YouTube Studio. A API usada pelo yutu nao oferece essas duas alteracoes.
...
```powershell
yutu watermark set --channelId $ChannelId --file "$Root\operations\a-palavra-que-cuida\04_assets\branding\marca-dagua-youtube-v1.png" --inVideoPosition bottomRight --offsetType offsetFromStart --offsetMs 30000 --yes
```
