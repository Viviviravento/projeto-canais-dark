...
def exploratory_theses(supplied: ChannelThesis, alternatives: list[ChannelThesis]) -> list[ChannelThesis]:
    ...
    return [
        ChannelThesis(**{**thesis.__dict__, "depth": "deep" if index == 0 else "scan"})
        for index, thesis in enumerate(unique)
    ]


def thesis_ready_for_pilot(thesis: ChannelThesis) -> bool:
