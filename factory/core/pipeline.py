...
_KIND_ALIASES = {"channel": "operation", "video": "content"}
...
class FactoryRun:
    """Checkpoint explícito para uma execução de operação ou conteúdo."""

    def __init__(self, run_id: str, kind: Literal["operation", "content", "channel", "video"], metadata: dict[str, Any] | None = None):
        kind = _KIND_ALIASES.get(kind, kind)
        if kind not in {"operation", "content"}:
            raise ValueError(f"Tipo de execução inválido: {kind}")
        self.run_id = run_id
        self.kind = kind
        self.created_at = utc_now()
        self.updated_at = self.created_at
        self.metadata = metadata or {}
        stage_ids = OPERATION_STAGES if kind == "operation" else CONTENT_STAGES
        self.stages = {stage_id: StageRecord(stage_id) for stage_id in stage_ids}

    @property
    def required_stages(self) -> tuple[str, ...]:
        return OPERATION_STAGES if self.kind == "operation" else CONTENT_STAGES
...
def validate_run_payload(payload: dict[str, Any], *, require_terminal: bool = False) -> None:
    ...
    required = OPERATION_STAGES if kind == "operation" else CONTENT_STAGES
